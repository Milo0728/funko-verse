...
@Component({
  ...
  template: `
    <header
      class="sticky top-0 z-50 border-b border-violet-500/10 bg-slate-950/70 backdrop-blur-xl"
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-6">
        <div class="flex items-center justify-between h-16 gap-4">
          ...
          <div class="flex items-center gap-2">
            <a
              routerLink="/products"
              class="hidden md:flex p-2 text-slate-300 hover:text-white hover:bg-white/5 rounded-lg transition"
              aria-label="Buscar"
            >
            ...
            @if (auth.isLogged()) {
              <a
                routerLink="/wishlist"
                class="hidden md:flex p-2 text-slate-300 hover:text-rose-400 hover:bg-white/5 rounded-lg transition"
            ...
            @if (!auth.isLogged()) {
              <a
                ...
                class="hidden sm:inline-flex fv-btn fv-btn-primary text-sm"
              >
                Entrar
              </a>
            } @else {
              <div class="relative">
                <button
                  type="button"
                  (click)="toggleMenu()"
                  class="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/5 transition"
                >
                  <span
                  ...
                  <span class="hidden md:inline text-sm text-slate-200 font-medium">
                    {{ auth.appUser()?.displayName ?? auth.firebaseUser()?.email }}
                  </span>
                </button>
                @if (menuOpen()) {
                  <div
                    class="absolute right-0 mt-2 w-56 fv-glass rounded-xl py-2 shadow-2xl fv-fade-in"
                  >
                    <a
                      routerLink="/profile"
                      (click)="closeMenu()"
          ...
          <nav class="lg:hidden pb-4 space-y-1 fv-fade-in">
            ...
            @if (!auth.isLogged()) {
              <a
                routerLink="/auth/login"
                (click)="closeMobile()"
                class="block px-3 py-2 text-violet-300 hover:bg-white/5 rounded-lg"
              >Entrar</a>
            }
          </nav>
        }
